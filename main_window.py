...
class Window:
    ...
    def show_intro_window(self):
        ...
        self.items = StringVar(self.window)
        self.items.set(self.options[0])
        self.dropdown = OptionMenu(self.window, self.items, *self.options)
        self.dropdown.config(width=15)
        self.dropdown.place(x = 704, y = 265)

        self.check = Checkbutton(bg = "#E4E4E4")
    ...
    def show_setup_window(self):
        ...
        self.text_algo = self.canvas.create_text(119.0, 247.5, text = "Value Iteration", fill = "#ffffff", font = ("RobotoRoman-Bold", 15))
        self.text_state = self.canvas.create_text(118.5, 295.5, text = "Deterministic", fill = "#ffffff", font = ("RobotoRoman-Bold", 15))
        ...
        self.b0.place(x = 20, y = 359, width = 197, height = 35)

        self.options = ["Agent", "Goal", "Trap", "Obstacle"]
        self.items = StringVar(self.window)
        self.items.set("Choose Item")
        self.dropdown = OptionMenu(self.window, self.items, *self.options)
        self.dropdown.config(width=15)
        self.dropdown.place(x = 60, y = 60)
    ...
    def show_main_window(self):
        ...
        self.text_algo = self.canvas.create_text(119.0, 257.5, text = "Value Iteration", fill = "#ffffff", font = ("RobotoRoman-Bold", 15))
        self.text_state = self.canvas.create_text(119.5, 305.5, text = "Deterministic", fill = "#ffffff", font = ("RobotoRoman-Bold", 15))

        self.grid_canvas = Canvas(self.canvas, bg = "#E4E4E4", height=400, width=400, bd = 0, highlightthickness = 0, relief = "ridge")
        self.grid_canvas.place(x = 284, y = 50)

        self.output_canvas = Canvas(self.canvas, bg = "#E4E4E4", height=400, width=250, bd = 0, highlightthickness = 0, relief = "ridge")
